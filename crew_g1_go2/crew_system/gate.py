# crew_g1_go2/crew_system/gate.py
"""Runtime contract gate for the real-robot path.

The mock-eval path checks plans inside orchestrator.execute_plan; this module
brings the same contract checks to the CrewAI/hardware pipeline. run_mission
arms the gate with the checked routing plan, and every dispatching tool asks
authorize() before touching a controller -- so no unchecked action reaches a
robot even if an execution agent drifts off the approved plan.
...
_IDLE = {"g1_busy": False, "go2_busy": False}
...
class ContractGate:
    ...
    def _args_match(step, skill, args):
        if skill == "navigate_to_location":
            want, got = step.args.get("location"), args.get("location")
            return (isinstance(want, str) and isinstance(got, str)
                    and want.lower().strip() == got.lower().strip())
        if skill == "navigate_to_coord":
            try:
